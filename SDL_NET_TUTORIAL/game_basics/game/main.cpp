...
void parse_packet(std::string from_server, int & x, int & y) {
	...
	for (int i = 0; i < from_server.length(); i++) {
		if (from_server[i] == 'p') {
			...
			while (from_server[i] != ',') {
				int c = 0;
				x_pos[c] = from_server[i];
				i++;
			}
			
			while (from_server[i] != ';') {
				int c = 0;
				y_pos[c] = from_server[i];
				i++;
			}
...
void send_data(std::string data, TCPsocket & server) {
	SDLNet_TCP_Send(server, data.c_str(), data.length());
}

std::string receive_data(TCPsocket & server) {
	char buffer[100];
	std::string data;
...
void space_game() 
{
	...
	while (1)
	{
		...
		surface.lock();
		surface.fill(BLACK);
		surface.put_rect(player, player_color);
		surface.unlock();
		surface.flip();
